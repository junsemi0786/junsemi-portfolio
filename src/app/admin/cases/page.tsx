    ...
    <main className={styles.main}>
      <div className="container">
        <div className={styles.header}>
          <h1 className="text-gradient">프로젝트 관리</h1>
          <Button href="/admin/cases/new">+ 프로젝트 추가</Button>
        </div>
              ...
              {cases.map((c) => (
                <tr key={c.id} className={styles.tr}>
                  ...
                  <td className={styles.tdStatus}>
                    <span
                      className={`${styles.statusBadge} ${c.status === "published" ? styles.statusPublished : styles.statusDraft}`}
                    >
                      {c.status === "published" ? "공개" : "초안"}
                    </span>
                  </td>
